...
typedef NSString* INSFeature NS_EXTENSIBLE_STRING_ENUM;
#if (TARGET_OS_IOS)
#ifndef Insights_EXCLUDE_PUSHNOTIFICATIONS
extern INSFeature const INSPushNotifications;
#endif
extern INSFeature const INSCrashReporting;
extern INSFeature const INSAutoViewTracking;
 ...
 */
extern NSString* const INSDefaultDeviceID;

/**
 * Use this as device ID for keeping all requests on hold until the real device ID is set later.
 * @discussion It can be used as @c deviceID on initial configuration, or passed as an argument for @c deviceID parameter on @c setNewDeviceID:onServer: method.
 * @discussion As long as device ID is @c INSTemporaryDeviceID, all requests will be on hold, but they will be persistently stored.
 ...
 * @discussion When in @c INSTemporaryDeviceID mode, method calls for presenting feedback widgets and updating remote config will be ignored.
 */
extern NSString* const INSTemporaryDeviceID;

//NOTE: Device ID Types
typedef NSString* INSDeviceIDType NS_EXTENSIBLE_STRING_ENUM;
extern INSDeviceIDType const INSDeviceIDTypeCustom;
extern INSDeviceIDType const INSDeviceIDTypeTemporary;
...
extern INSConsent const INSConsentEvents;
extern INSConsent const INSConsentUserDetails;
extern INSConsent const INSConsentCrashReporting;
extern INSConsent const INSConsentPushNotifications;
...
typedef NSString* INSPushTestMode NS_EXTENSIBLE_STRING_ENUM;
extern INSPushTestMode const INSPushTestModeDevelopment;
extern INSPushTestMode const INSPushTestModeTestFlightOrAdHoc;

//NOTE: Default metrics
typedef NSString* INSMetricKey NS_EXTENSIBLE_STRING_ENUM;
extern INSMetricKey const INSMetricKeyDevice;
extern INSMetricKey const INSMetricKeyDeviceType;
extern INSMetricKey const INSMetricKeyOS;
extern INSMetricKey const INSMetricKeyOSVersion;
extern INSMetricKey const INSMetricKeyAppVersion;
extern INSMetricKey const INSMetricKeyCarrier;
extern INSMetricKey const INSMetricKeyResolution;
extern INSMetricKey const INSMetricKeyDensity;
extern INSMetricKey const INSMetricKeyLocale;
...
@interface InsightsConfig : NSObject
...
/**
 * Application's App Key found on Insights Server's "Management > Applications" section.
...
/**
 * For specifying which features Insights will start with.
 * @discussion Available features for each platform:
 * @discussion @b iOS:
 * @discussion @c INSPushNotifications for push notifications
 * @discussion @c INSCrashReporting for crash reporting
 * @discussion @c INSAutoViewTracking for auto view tracking
 ...
 * @discussion @c INSCrashReporting for crash reporting
 */
@property (nonatomic, copy) NSArray<INSFeature>* features;
 ...
 * @discussion For overriding default metrics, keys should be @c INSMetricKey 's.
 */
 ...
 */
@property (nonatomic, copy) NSArray<INSConsent>* consents;
#pragma mark -
 ...
 * @discussion - @c INSPushTestModeTestFlightOrAdHoc: For TestFlight or AdHoc builds signed with a distribution provisioning profile. Insights Server will send push notifications to Production APNs.
 * @discussion If set, Test Users mark should be selected on Create Push Notification screen of Insights Server to send push notifications.
 * @discussion If not set (or set to @c nil ), Insights Server will use Production APNs by default.
 */
@property (nonatomic, copy) INSPushTestMode _Nullable pushTestMode;

/**
 * For sending push tokens to Insights Server even for users who have not granted permission to display notifications.
 ...
 * @discussion Needs to be set in @c applicationDidFinishLaunching: method of macOS apps that use @c INSPushNotifications feature, in order to handle app launches by push notification click.
 */
 ...
 * @discussion By default, Insights Server uses a geo-ip database for acquiring user's location. If the app uses Core Location services and granted permission, a location with better accuracy can be provided using this property.
 ...
 * @discussion By default, Insights Server uses a geo-ip database for acquiring user's location. If the app has information about user's city, it can be provided using this property.
 * @discussion It will be sent with @c begin_session requests only.
 */
 ...
 * @discussion By default, Insights Server uses a geo-ip database for acquiring user's location. If the app has information about user's country, it can be provided using this property.
 * @discussion It will be sent with @c begin_session requests only.
 */
 ...
 * @discussion By default, Insights Server uses a geo-ip database for acquiring user's location, and deduces the IP address from the connection. If the app needs to explicitly specify the IP address due to network requirements, it can be provided using this property.
 * @discussion It will be sent with @c begin_session requests only.
 */
 ...
 * @discussion If set, persistently stored device ID will be reset and new device ID specified on @c deviceID property of @c InsightsConfig object will be stored and used.
...
/**
 * Update session period is used for updating sessions and sending queued events to Insights Server periodically.
...
/**
 * Stored requests limit is used for limiting the number of request to be stored on the device, in case Insights Server is not reachable.
 * @discussion In case Insights Server is down or unreachable for a very long time, queued request may reach excessive numbers, and this may cause problems with requests being sent to Insights Server and being stored on the device. To prevent this, SDK will only store requests up to @c storedRequestsLimit.
...
/**
 * For using custom crash segmentation with @c INSCrashReporting feature.
 * @discussion Crash segmentation should be an @c NSDictionary, with keys and values are both @c NSString's only.
 * @discussion Custom objects in crash segmentation will cause crash report not to be sent to Insights Server.
 * @discussion Nested values in crash segmentation will be ignored by Insights Server.
 */
...
/**
 * Regular expression used for filtering crash reports and preventing them from being sent to Insights Server.
 * @discussion If a crash's name, description or any line of stack trace matches given regular expression, it will not be sent to Insights Server.
 */
...
/**
 * Callback block to decide whether the crash report detected by PLCrashReporter should be sent to Insights Server or not.
 * @discussion If not set, crash report will be sent to Insights Server by default.
 * @discussion If set, crash report will be sent to Insights Server only if `YES` is returned.
 * @discussion It has an @c NSDictionary parameter that represents crash report object.
 * @discussion If @c shouldUsePLCrashReporter flag is not set on initial config, it will never be executed.
 */
 ...
 * @discussion If set, every request sent to Insights Server will have @c checksum256 value generated by SHA256(request + secretSalt)
 */
 ...
 * @discussion Completion block has a single NSInteger parameter that indicates 1 to 5 star-rating given by user. If user dismissed dialog without giving a rating, this value will be 0 and it will not be reported to Insights Server.
 */
 ...
 * @discussion If set, Remote Config values specified on Insights Server will be fetched on beginning of sessions.
 */
@property (nonatomic) BOOL enableRemoteConfig;

/**
 * Completion block to be executed after remote config is fetched from Insights Server, on start or device ID change.
 ...
 * @discussion If Insights Server is not reachable or if there is another error, it will be executed with an @c NSError indicating the problem.
